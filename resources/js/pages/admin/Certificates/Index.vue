...
function buildWeeklyEmail(group: WeeklyGroup): string {
  ...
  const parentScript = `"Hi [Parent Name], I've recently partnered with musicExams.help, a platform that supports teachers, parents and students taking Trinity exams. Your child now receives a personalised certificate (in addition to any official Trinity certificate) — please find it attached. They also appear on the Recognition page at https://musicexams.help/recognition (first name and surname initial only). They're also in the running for our quarterly Top Scorer award — winners receive a gift token, announced around 6 weeks after the quarter ends once all results (including digital) are in. So we can credit the right person, please reply with your child's music teacher's name — and if their lessons are through a music school, let us know which one. If you'd like their full name displayed, just email musicexams@musicexams.help."`
