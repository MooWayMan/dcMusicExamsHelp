    ...
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div class="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div class="rounded-xl border border-brand-border bg-brand-surface p-4">
                <p class="text-sm font-medium text-brand-text-soft">Orders</p>
                <p class="mt-1 text-2xl font-bold text-brand-text">{{ summary.total_orders }}</p>
            </div>
            <div class="rounded-xl border border-brand-border bg-brand-surface p-4">
                <p class="text-sm font-medium text-brand-text-soft">Total Commission</p>
                <p class="mt-1 text-2xl font-bold text-brand-success">&pound;{{ summary.total_commission }}</p>
            </div>
            <div class="rounded-xl border border-brand-border bg-brand-surface p-4">
                <p class="text-sm font-medium text-brand-text-soft">Total Candidates</p>
                <p class="mt-1 text-2xl font-bold text-brand-text">{{ summary.total_candidates }}</p>
            </div>
        </div>
        ...
        <div class="mt-4 overflow-hidden rounded-xl border border-brand-border bg-brand-surface">
                ...
                <table class="w-full text-left text-sm">
                    ...
                    <tbody class="divide-y divide-brand-border">
                        <tr v-for="order in orders.data" :key="order.id" class="transition-colors hover:bg-brand-surface-soft">
                            ...
                            <td class="px-4 py-3">
                                <Link v-if="order.teacher_id" :href="`/admin/teachers/${order.teacher_id}`" class="text-sm text-brand-text hover:text-brand-accent hover:underline">
                                    {{ order.teacher_name }}
                                </Link>
                                <span v-else class="text-sm text-brand-text-soft">{{ order.teacher_name }}</span>
                            </td>
                            <td class="hidden px-4 py-3 text-sm text-brand-text-soft lg:table-cell">{{ order.school_name }}</td>
                            <td class="px-4 py-3 text-center">
                                <span class="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium"
                                    ...
                                    <Monitor v-if="order.delivery_method === 'DG'" class="h-3 w-3" />
                                    <MapPin v-else class="h-3 w-3" />
                                    {{ order.delivery_method }}
                                </span>
                            </td>
                            <td class="hidden px-4 py-3 text-sm text-brand-text-soft md:table-cell">{{ order.subject_area }}</td>
                            <td class="px-4 py-3 text-center text-sm text-brand-text">{{ order.candidates }}</td>
                            <td class="px-4 py-3 text-right text-sm font-medium text-brand-success">&pound;{{ order.commission_amount }}</td>
                            <td class="px-4 py-3 text-center">
                        ...
                        <tr v-if="!orders.data.length">
                            <td colspan="9" class="px-4 py-8 text-center text-brand-text-soft">No orders found.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div v-if="orders.last_page > 1" class="flex items-center justify-between border-t border-brand-border px-4 py-3">
                <p class="text-sm text-brand-text-soft">Page {{ orders.current_page }} of {{ orders.last_page }}</p>
                <div class="flex gap-1">
